...
format:
  html:
    other-links:
      ...
      - text: "FPP3 §13.3: Ensuring forecasts stay within limits"
        href: https://otexts.com/fpp3/limits.html
  revealjs:
...
```{r}
#| label: tourism-plot-html-render
#| renderings: [light, dark]
#| echo: false

ggplotly(tourism_plot_html_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(tourism_plot_html_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: stl-decomp-render
#| renderings: [light, dark]
#| echo: false

stl_decomp_p + theme_narsil()
stl_decomp_p + theme_narsil_dark()
```
...
```{r}
#| label: plot-with-gaps-render
#| renderings: [light, dark]
#| echo: false

ggplotly(plot_with_gaps_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(plot_with_gaps_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
plot_imputed_p <- ah |>
  autoplot(Trips) +
  autolayer(ah_fill, Trips, colour = "#5B8DB8") +
  geom_point(                             #<2>
    data  = ah_imputed_pts,
...
```{r}
#| label: plot-imputed-render
#| renderings: [light, dark]
#| echo: false

ggplotly(plot_imputed_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(plot_imputed_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: eggs-with-line-html-render
#| renderings: [light, dark]
#| echo: false

eggs_with_line_html_p + scale_narsil()
eggs_with_line_html_p + scale_narsil_dark()
```
...
```{r}
#| label: eggs-log-forecast-render
#| renderings: [light, dark]
#| echo: false

eggs_log_forecast_p + scale_narsil()
eggs_log_forecast_p + scale_narsil_dark()
```
...
```{r}
#| label: eggs-scaled-logit-render
#| renderings: [light, dark]
#| echo: false

eggs_scaled_logit_p + scale_narsil()
eggs_scaled_logit_p + scale_narsil_dark()
```
