...
`prophet` is not part of the initial course setup. If you have not installed it
yet, run this once in the console:

```r
pak::pak("fable.prophet")
```

See [Setup](../../../more/r-tools/setup.qmd) if you run into trouble.
:::
...
```{r}
#| label: lax-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(lax_plot_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(lax_plot_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: lax-seasonal-render
#| renderings: [light, dark]
#| echo: false

lax_seasonal_p + scale_narsil() + theme(legend.position = "top")
lax_seasonal_p + scale_narsil_dark() + theme(legend.position = "top")
```
...
```{r}
#| label: lax-forecast-render
#| renderings: [light, dark]
#| echo: false

lax_forecast_p + scale_narsil() + theme(legend.position = "top")
lax_forecast_p + scale_narsil_dark() + theme(legend.position = "top")
```
...
```{r}
#| label: lax-comparison-render
#| renderings: [light, dark]
#| echo: false

ggplotly(lax_comparison_p + scale_narsil()) |> gg_to_plotly_narsil()
ggplotly(lax_comparison_p + scale_narsil_dark()) |> gg_to_plotly_narsil(dark = TRUE)
```
