...
We cannot observe $\beta_0$ and $\beta_1$ directly — we estimate them from data. The **Ordinary Least Squares (OLS)** method chooses the estimates $\hat{\beta}_0$ and $\hat{\beta}_1$ that minimize the **sum of squared residuals**:

$$
\min_{\hat{\beta}_0, \hat{\beta}_1} \sum_{t=1}^{T} \hat{\varepsilon}_t^2 = \sum_{t=1}^{T} \left(y_t - \hat{\beta}_0 - \hat{\beta}_1 x_t\right)^2
$$

Taking partial derivatives and setting them to zero yields the **normal equations**, whose solution gives:

$$
\hat{\beta}_1 = \frac{\sum x_t y_t - T\bar{x}\bar{y}}{\sum x_t^2 - T\bar{x}^2}, \qquad \hat{\beta}_0 = \bar{y} - \hat{\beta}_1 \bar{x}
$$

In practice, R handles this for us. But knowing the objective function matters: OLS penalizes **large** residuals more than small ones (because of the square), and it produces estimators with desirable statistical properties under the classical assumptions — the Gauss-Markov theorem guarantees they are **BLUE** (Best Linear Unbiased Estimators), provided those assumptions hold.
:::
...
Keep this in mind for the rest of the module — and, frankly, for the rest of your career.
...
Most students assume linear regression can only produce straight lines. Let's challenge that.

*Are these models linear?*

$$
y_t = \beta_0 + \beta_1 x_t + \beta_2 x_t^2 + \varepsilon_t \qquad \text{(quadratic)}
$$

$$
\log y_t = \beta_0 + \beta_1 x_t + \varepsilon_t \qquad \text{(log-linear)}
$$

$$
y_t = \beta_0 + \beta_1 x_t + \beta_2 x_t^2 + \beta_3 x_t^3 + \varepsilon_t \qquad \text{(cubic)}
$$

:::{.fragment}

**Yes — all of them are linear regression models.**
...
2. **Linearity in the parameters** — $E(y \mid x)$ is linear in the $\beta$'s. This is what matters for OLS.

:::

:::{.fragment}

The three models above are all **nonlinear in $x$** but **linear in $\beta$** — so they can all be estimated by OLS. A linear regression model can produce a straight line, a parabola, an exponential curve, or a piecewise function, depending on the **functional form** chosen.
...
p <- bind_rows(us_change_aug_simple, us_change_aug_mult) |>
  ggplot(aes(x = Quarter)) +
  geom_line(aes(y = Consumption), color = "grey50") +
  geom_line(aes(y = .fitted, color = .model)) +
  facet_wrap(~.model, ncol = 1) +
  labs(y = "% change", x = NULL, color = "Model") +
  theme(legend.position = "none")
...
```{r}
#| label: us-change-compare-scatter
#| echo: false

bind_rows(us_change_aug_simple, us_change_aug_mult) |>
...
```{r}
#| label: us-change-mult-resid-pred
#| echo: false
...
```{r}
#| label: us-change-mult-lb

us_change_dof_mult <- us_change_fit_mult |>
  tidy() |>
  nrow()
