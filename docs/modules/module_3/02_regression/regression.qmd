...
```{r}
#| label: us-change-time-render
#| renderings: [light, dark]
#| echo: false

ggplotly(us_change_time_p + scale_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(us_change_time_p + scale_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
The stars (`*`, `**`, `***`) indicate $p < 0.05$, $p < 0.01$, $p < 0.001$. They
rank how cleanly a coefficient separates from zero, not how large it is — see
[Statistical Significance](../../../more/significance.qmd).
...
us_change_simple_aug_p <- augment(us_change_fit_simple) |>
...
```{r}
#| label: us-change-simple-aug-render
#| renderings: [light, dark]
#| echo: false

ggplotly(us_change_simple_aug_p + scale_narsil() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(us_change_simple_aug_p + scale_narsil_dark() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
us_change_simple_scatter_p <- augment(us_change_fit_simple) |>
  ggplot(aes(x = Consumption, y = .fitted)) +
  geom_point(alpha = 0.6) +
  geom_abline(intercept = 0, slope = 1, colour = "#D55E00") +
  labs(x = "Actual", y = "Fitted",
       title = "Points close to the 45° line = good fit")
```

```{r}
#| label: us-change-simple-scatter-render
#| renderings: [light, dark]
#| echo: false

us_change_simple_scatter_p + theme_narsil()
us_change_simple_scatter_p + theme_narsil_dark()
```
...
us_change_compare_time_html_p <- us_change_aug_both |>
  ggplot(aes(x = Quarter)) +
...
```{r}
#| label: us-change-compare-time-html-render
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "Grey line = actual consumption; colored line = fitted values. The multiple model tracks the data much more closely."
#| lightbox:
...
ggplotly(us_change_compare_time_html_p + scale_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(us_change_compare_time_html_p + scale_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
us_change_compare_scatter_html_p <- us_change_aug_both |>
...
```{r}
#| label: us-change-compare-scatter-html-render
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "Points closer to the 45° line indicate better fit. The multiple model shows a much tighter cloud."
#| lightbox:
...
ggplotly(us_change_compare_scatter_html_p + scale_narsil() + theme(legend.position = "none")) |> gg_to_plotly_narsil()
ggplotly(us_change_compare_scatter_html_p + scale_narsil_dark() + theme(legend.position = "none")) |> gg_to_plotly_narsil(dark = TRUE)
```
...
us_change_resid_pred_p <- left_join(
  us_change,
...
```{r}
#| label: us-change-resid-pred-render
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "Residuals from the multiple model vs. each predictor. No strong patterns suggest the linear specification is adequate."
#| lightbox:
...
us_change_resid_pred_p + theme_narsil()
us_change_resid_pred_p + theme_narsil_dark()
```
...
```{r}
#| label: us-change-exante-render
#| renderings: [light, dark]
#| echo: false

us_change_exante_p + theme_narsil()
us_change_exante_p + theme_narsil_dark()
```
...
```{r}
#| label: us-change-expost-render
#| renderings: [light, dark]
#| echo: false

us_change_expost_p + theme_narsil()
us_change_expost_p + theme_narsil_dark()
```
...
```{r}
#| label: us-change-scenarios-render
#| renderings: [light, dark]
#| echo: false

us_change_scenarios_p + theme_narsil()
us_change_scenarios_p + theme_narsil_dark()
```
...
## Can regression handle trend and seasonality?
...
```{r}
#| label: beer-plot-render
#| renderings: [light, dark]
#| echo: false

beer_plot_p + theme_narsil()
beer_plot_p + theme_narsil_dark()
```
...
## Trend and seasonal dummies in R
...
beer_fitted_p <- augment(beer_fit) |>
  ggplot(aes(x = Quarter)) +
...
```{r}
#| label: beer-fitted-render
#| renderings: [light, dark]
#| echo: false

ggplotly(beer_fitted_p + scale_narsil() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(beer_fitted_p + scale_narsil_dark() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
## Intervention Variables and Outliers
...
The fix: **decompose first, then apply IQR to the remainder**. For `beer`, which has strong known seasonality, we use a full STL decomposition — including the seasonal component — so only genuine anomalies end up in the remainder^[When seasonality is weak or absent, `season(period = 1)` can be used to extract only the trend. See [Practical Forecasting Issues](../01_practical_issues/practical_issues.qmd) for the full workflow including NA imputation.].
 
:::
 
```{r}
#| label: beer-stl-outlier
...
```{r}
#| label: beer-outlier-plot
#| echo: false
#| output: false

beer_outlier_plot_p <- beer |>
  autoplot(Beer) +
  geom_point(
    data = beer |> semi_join(beer_outliers |> filter(outlier_15), by = "Quarter"),
    aes(y = Beer),
    colour = "#D55E00", size = 3
  ) +
  labs(title = "Beer production: outliers detected via STL remainder (c = 1.5)",
       y = "Megalitres", x = NULL)
```

```{r}
#| label: beer-outlier-plot-render
#| renderings: [light, dark]
#| echo: false

beer_outlier_plot_p + theme_narsil()
beer_outlier_plot_p + theme_narsil_dark()
```
...
beer_intervention_plot_p <- augment(beer_fit_interv) |>
  ggplot(aes(x = Quarter)) +
  geom_line(aes(y = Beer, color = "Actual")) +
  geom_line(aes(y = .fitted, color = "Fitted")) +
  labs(y = "Megalitres", color = NULL,
...
```{r}
#| label: beer-intervention-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(beer_intervention_plot_p + scale_narsil() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(beer_intervention_plot_p + scale_narsil_dark() + theme(legend.position = "top"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: boston-data-render
#| renderings: [light, dark]
#| echo: false

boston_data_p + theme_narsil()
boston_data_p + theme_narsil_dark()
```
 
:::{.content-hidden unless-format="revealjs"}
 
## A linear trend first
...
```{r}
#| label: boston-linear-only-render
#| renderings: [light, dark]
#| echo: false

boston_linear_only_p + theme_narsil()
boston_linear_only_p + theme_narsil_dark()
```
...
```{r}
#| label: boston-fit-render
#| renderings: [light, dark]
#| echo: false

boston_fit_p + scale_narsil() + theme(legend.position = "top")
boston_fit_p + scale_narsil_dark() + theme(legend.position = "top")
```
...
```{r}
#| label: boston-knots-render
#| renderings: [light, dark]
#| echo: false

boston_knots_p + scale_narsil() + theme(legend.position = "top")
boston_knots_p + scale_narsil_dark() + theme(legend.position = "top")
```
...
| `trend() + season()` | Deterministic patterns |
