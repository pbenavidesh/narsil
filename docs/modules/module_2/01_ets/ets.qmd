...
1. The setup `ETS(y ~ error("M") + trend("Ad") + season("M"))` is often a robust choice for seasonal data with trend.
...
Table: ETS component combinations (trend × seasonal) {#tbl-ets-grid}
...
Table: Names of some popular ETS models {#tbl-ets-names}
...
- The choice of model components (error, trend, seasonality) should be based on the characteristics of the data.
