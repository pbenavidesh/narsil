...
google_2015 |>
  gg_tsdisplay(difference(Close), plot_type = "partial") #<1>
```
...
google_2015 |>
  gg_tsdisplay(difference(Close, differences = 2), plot_type = "partial") #<1>
```
...
h02 |>
  gg_tsdisplay(difference(log(Cost), lag = 12), #<1>
               plot_type = "partial",
               lag_max = 48)
```
...
Applying the same idea twice:

$$y''_t = y'_t - y'_{t-1} = (1-B)y'_t = (1-B)(1-B)y_t = (1-B)^2 y_t$$

Expanding $(1-B)^2$ gives a **perfect square trinomial**:

$$(1-B)^2 y_t = (1 - 2B + B^2)y_t = y_t - 2y_{t-1} + y_{t-2}$$

:::{.content-visible unless-format="revealjs"}
![](trinomio cuadrado perfecto.jpg){width=50%}
:::
