---
title: "Stationarity & Differencing"
format:
  html: default
  revealjs:
    output-file: stationarity_pres.html
params:
  fred-data: true
---

```{r}
#| label: pkgs
#| message: false
#| include: false

library(tidyverse)
library(fpp3)
library(ggtime)
library(patchwork)

source(here::here("R/narsil_theme.R"))
theme_set(theme_narsil())
```

:::{.content-visible unless-format="revealjs"}

```{r}
#| label: pkgs_special
#| message: false

library(tidyquant) #<1>
library(plotly)    #<1>
```

1. In addition to the regular packages, here we'll use `tidyquant` and `plotly`.

:::

```{r}
#| label: data-setup
#| include: false
#| message: false

mexretail <- tq_get(
  "MEXSLRTTO01IXOBM",
  get = "economic.data",
  from = "1985-01-01"
) |>
  mutate(date = yearmonth(date)) |>
  rename(y = price) |>
  as_tsibble(index = date)

lambda <- mexretail |>
  features(y, features = guerrero) |>
  pull(lambda_guerrero)

google_stock <- gafa_stock |>
  filter(Symbol == "GOOG") |>
  mutate(day = row_number()) |>
  update_tsibble(index = day, regular = TRUE) |>
  mutate(diff_close = difference(Close))

google_2015 <- google_stock |> filter(year(Date) == 2015)
```


# Stationarity

::: {.content-hidden unless-format="revealjs"}
## What Does "Stationary" Mean? 
:::

Have you ever heard the word **"stationary"** before?

::: {.columns}

::: {.column width="50%"}

:::{.fragment}

![Gas cylinders & stationary tanks — what's the difference?](gas_tanks.jpg){width=50%}

:::
:::

::: {.column width="50%"}
::: {.fragment}
- In everyday language, **stationary** means *not moving* — fixed in place.
- For a time series, what would that mean?
:::
:::

:::

:::{.fragment}
:::{.callout-note appearance="simple"}
## Think about it

If a time series is "not moving", what statistical properties would you expect it to have?
:::
:::

## The Formal Definition

A time series is **stationary** if its statistical properties — primarily its **mean** and **variance** — do not change over time.

- The series fluctuates around a **constant mean**.
- The spread of those fluctuations stays **roughly constant** over time.
- There are no systematic patterns that change the level or scale of the series.


## Are These Series Stationary?

Which of the following six series are stationary (i.e., which look more *stable*)?

:::{.content-visible unless-format="revealjs"}

```{r}
#| label: all-six-google
#| echo: true
#| message: false
#| output: false

p_google <- google_2015 |>
  autoplot(Close) +
  labs(title = "(a) Google stock price", y = "USD", x = "")
```

```{r}
#| label: all-six-google-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_google + theme_narsil()
p_google + theme_narsil_dark()
```

```{r}
#| label: all-six-eggs
#| echo: true
#| message: false
#| output: false

eggs <- as_tsibble(fma::eggs)
p_eggs <- eggs |>
  autoplot(value) +
  labs(title = "(b) Price of a dozen eggs (US)", y = "Constant US cents", x = "")
```

```{r}
#| label: all-six-eggs-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_eggs + theme_narsil()
p_eggs + theme_narsil_dark()
```

```{r}
#| label: all-six-beer
#| echo: true
#| message: false
#| output: false

recent_production <- aus_production |>
  filter(year(Quarter) >= 1992 & year(Quarter) <= 1995)
p_beer <- recent_production |>
  autoplot(Beer) +
  labs(title = "(c) Australian beer production", y = "Megalitres", x = "")
```

```{r}
#| label: all-six-beer-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_beer + theme_narsil()
p_beer + theme_narsil_dark()
```

```{r}
#| label: all-six-pigs
#| echo: true
#| message: false
#| output: false

pigs <- aus_livestock |>
  filter(Animal == "Pigs", State == "Victoria")
p_pigs <- pigs |>
  autoplot(Count) +
  labs(title = "(d) Pigs slaughtered (Victoria)", y = "Count", x = "")
```

```{r}
#| label: all-six-pigs-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_pigs + theme_narsil()
p_pigs + theme_narsil_dark()
```

```{r}
#| label: all-six-returns
#| echo: true
#| message: false
#| output: false

p_returns <- google_2015 |>
  autoplot(diff_close) +
  labs(title = "(e) Google stock daily returns", y = "Δ USD", x = "")
```

```{r}
#| label: all-six-returns-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_returns + theme_narsil()
p_returns + theme_narsil_dark()
```

```{r}
#| label: all-six-lynx
#| echo: true
#| message: false
#| output: false

lynx <- pelt |> select(Year, Lynx)
p_lynx <- lynx |>
  autoplot(Lynx) +
  labs(title = "(f) Lynx pelts traded", y = "Count", x = "")
```

```{r}
#| label: all-six-lynx-render
#| renderings: [light, dark]
#| echo: false
#| message: false

p_lynx + theme_narsil()
p_lynx + theme_narsil_dark()
```

:::

:::{.content-hidden unless-format="revealjs"}

```{r}
#| label: trend-series-pres
#| echo: false
#| layout-ncol: 3
#| lightbox:
#|   group: "six-series-pres"
#| results: hide
theme_narsil()
p_google
p_eggs
p_beer
p_pigs
p_returns
p_lynx
```


:::

:::{.content-visible unless-format="revealjs"}

:::{.callout-tip collapse="true"}
## Answer

- **(a)** and **(b)**: clear trends (upward and downward) → **not stationary** ✗
- **(c)** and **(d)**: strong repeating seasonal pattern → **not stationary** ✗
- **(e)**: daily returns fluctuate around zero with constant spread → **stationary** ✓
- **(f)**: the cycling looks seasonal but has no fixed period — this is a *business cycle*. The series is **stationary** ✓

**Key distinction:** Seasonality repeats at a fixed, known frequency. Business cycles rise and fall irregularly.
:::

:::

## What Makes a Series Non-Stationary?

A time series is **non-stationary** if it exhibits any of the following:

- **Trend** — a long-term increase or decrease in the mean.
- **Seasonality** — a repeating pattern that causes the mean to shift systematically.
- **Changing variance** — the spread of fluctuations grows or shrinks over time.

::: {.fragment}
::: {.callout-note}
## What can we expect from a Stationary Series then?
A stationary serie will not have any systematic patterns that change the level or scale of the series over time (**no trend**, **no seasonality**, and **constant variance**).  
:::
:::

## Why Does Stationarity Matter?

- There is an important family of forecasting models that work by describing the **correlation structure** between a series and its own past values.

- For those correlations to be stable and meaningful, the series needs to be stationary.

:::{.fragment}
:::{.callout-note appearance="simple"}
## The key question

- If these models require stationarity, does that mean they can *only* work with series that have no trend, no seasonality, and no changing variance?

- Or is there something we can do to a non-stationary series to make it usable?
:::
:::

## What to do with Non-Stationary Series?

::: {.fragment}
### If we find heteroskedasticity (changing variance)

Recall from [Module 1](../../module_1/02_ts_dcmp/ts_dcmp.qmd#mathematical-transformations): we can **stabilize the variance** using mathematical transformations — logarithms, Box-Cox, and so on.
:::


::: {.fragment}
### But what about the **mean**? 

Transformations alone don't remove a trend or seasonal pattern.
:::

::: {.content-visible unless-format="revealjs"}
# Differencing
:::

::: {.content-hidden unless-format="revealjs"}
##
:::

Look at these two series:


```{r}
#| label: google-reveal-render
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "The second series was produced directly from the first. Can you figure out how?"
#| lightbox:
#|   group: google-series

(p_google + theme_narsil()) | (p_returns + theme_narsil())
(p_google + theme_narsil_dark()) | (p_returns + theme_narsil_dark())
```



:::{.fragment}
:::{.callout-note }
## Differencing
The daily return is simply today's price minus yesterday's price — the **change** between consecutive observations. This operation is called **differencing**, and it is how we stabilize the mean of a non-stationary series.
:::

:::{.callout-tip collapse="true"}
## Random walks & stationarity

- Google's stock price follows a **random walk**. 
- By definition, random walks are non-stationary, but their first differences are stationary.
:::
:::

## First Differences

The **first difference** of a series $y_t$ is:

$$y'_t = y_t - y_{t-1}$$

- The differenced series has $T - 1$ observations.
- First differences represent the *change* from one period to the next.
- If the original series has a linear trend, the differences will fluctuate around a constant mean.

::: {.fragment}

```{r}
#| label: google-tsdisplay
#| warning: false
#| message: false
#| output: false

google_tsdisplay_p <- google_2015 |>
  autoplot(difference(Close)) #<1>
```
1. `difference(Close)` computes the first difference of the `Close` variable.

```{r}
#| label: google-tsdisplay-render
#| renderings: [light, dark]
#| echo: false

google_tsdisplay_p + theme_narsil()
google_tsdisplay_p + theme_narsil_dark()
```

:::

## Second Differences

Sometimes the first-differenced series is still non-stationary. We can **difference again**:

$$
\begin{align*}
y''_t &= y'_t - y'_{t-1} \\
      &= (y_t-y_{t-1}) - (y_{t-1} - y_{t-2}) \\
      &= y_t - 2y_{t-1} + y_{t-2}
\end{align*}
$$

- Second differences have $T - 2$ observations.
- They represent the *change in the changes* — acceleration rather than velocity.
- **Economic and business series almost never require more than two differences.**

:::{.fragment}
:::{.callout-warning appearance="simple"}
Needing three or more differences usually signals something else is wrong — an outlier, a structural break, or a transformation that should have been applied first.
:::

```{r}
#| label: google-second-diff
#| output: false

google_second_diff_p <- google_2015 |>
  autoplot(difference(Close, differences = 2)) #<1>
```
1. The argument `differences = 2` tells `difference()` to apply the differencing operation twice.

```{r}
#| label: google-second-diff-render
#| renderings: [light, dark]
#| echo: false

google_second_diff_p + theme_narsil()
google_second_diff_p + theme_narsil_dark()
```

:::




## Seasonal Differencing

If the series has seasonality, we take a **seasonal difference** — the change relative to the same period in the previous cycle:

$$y'_t = y_t - y_{t-m}$$

where $m$ is the seasonal period ($m = 12$ for monthly data, $m = 4$ for quarterly, ...).

- Seasonal differences represent *season-over-season change*^[most commonly YoY (year-over-year), but remember there can be daily, weekly, or more seasonal patterns too.] at each point.
- Also called **lag-$m$ differences**.
- After seasonal differencing, any remaining non-seasonal trend can be removed with a first difference.

::: {.fragment}

```{r}
#| label: pbs-setup
#| message: false
#| echo: false
#| output: false

h02 <- PBS |>
  filter(ATC2 == "H02") |>
  summarise(Cost = sum(Cost) / 1e6)

pbs_setup_p <- h02 |>
  autoplot(log(Cost))
```

```{r}
#| label: pbs-setup-render
#| renderings: [light, dark]
#| echo: false

pbs_setup_p + theme_narsil()
pbs_setup_p + theme_narsil_dark()
```

```{r}
#| label: h02-sdiff
#| warning: false
#| message: false
#| output: false

h02_sdiff_p <- h02 |>
  autoplot(difference(log(Cost), lag = 12)) #<1>
```
1. The argument `lag = 12` tells `difference()` to compute the seasonal difference with a lag of 12 periods (i.e. $y_t - y_{t-12}$). If no `lag` is specified, it defaults to 1 (the first difference).

```{r}
#| label: h02-sdiff-render
#| renderings: [light, dark]
#| echo: false

h02_sdiff_p + theme_narsil()
h02_sdiff_p + theme_narsil_dark()
```

:::

## Does the Order of Differencing Matter?

When a series needs both seasonal and regular differencing, does it matter which one we apply first?

Applying **seasonal first, then regular**:

$$
\begin{align*}
(y_t - y_{t-m})' &= (y_t - y_{t-m}) - (y_{t-1} - y_{t-m-1}) \\
                 &= y_t - y_{t-1} - y_{t-m} + y_{t-m-1}
\end{align*}
$$

::: {.fragment}

Applying **regular first, then seasonal**:

$$
\begin{align*}
(y_t - y_{t-1})' &= (y_t - y_{t-1}) - (y_{t-m} - y_{t-m-1}) \\
                 &= y_t - y_{t-1} - y_{t-m} + y_{t-m-1}
\end{align*}
$$

Both routes lead to the same result.

:::{.callout-tip appearance="simple"}
The order **does not matter** — the result is always identical. In practice, if the seasonal pattern is strong, apply the seasonal difference first: the result may already be stationary without needing the regular difference.
:::
:::


## Backshift Notation

The **backshift operator** $B$ provides a compact way to write differencing operations — and, as we will see next class, to write out full model equations cleanly.

It is defined simply as:

$$
By_t = y_{t-1} ; B(By_t) = B^2 y_t = y_{t-2}
$$


That is, applying $B$ to a series shifts it back one period.

::: {.fragment}

### First Differences

Recall the first difference:

$$
y'_t = y_t - y_{t-1}
$$

We can rewrite $y_{t-1} = By_t$, so:

$$
y'_t = y_t - By_t = (1 - B)y_t
$$
:::

::: {.fragment}

### Second Differences


Recall:

$$
y''_t = y_t - 2y_{t-1} + y_{t-2}
$$

Using the backshift operator turns to:

:::

::: {.fragment}

$$
y''_t = y_t - 2By_t + B^2 y_t = (1 - 2B + B^2)y_t
$$

:::

::: {.fragment}
which gives a **perfect square trinomial**^[![](trinomio-cuadrado-perfecto.png)]

$$
y''_t = (1-B)^2 y_t
$$

:::

:::{.content-hidden unless-format="revealjs"}
:::{.fragment}
![](trinomio-cuadrado-perfecto.png){width=60%}
:::
:::

::: {.fragment}

### General $d$-level Differences

The pattern generalizes naturally:

$$
(1 - B)^d y_t
$$

where $d$ is the number of times we difference the series.

:::{.content-visible unless-format="revealjs"}
:::{.callout-tip collapse="true"}
## Binomial expansion of $(1-B)^d$

For a general order $d$, the binomial theorem gives:

$$
(1-B)^d = \sum_{k=0}^{d} \binom{d}{k} (-B)^k = \sum_{k=0}^{d} \binom{d}{k} (-1)^k B^k
$$

So:

$$
(1-B)^d y_t = \sum_{k=0}^{d} \binom{d}{k} (-1)^k y_{t-k}
$$

For $d=1$: $y_t - y_{t-1}$ ✓  
For $d=2$: $y_t - 2y_{t-1} + y_{t-2}$ ✓
:::
:::

:::

::: {.fragment}

### Seasonal Differences

A seasonal difference shifts back $m$ periods:

$$
y_t - y_{t-m} = y_t - B^m y_t = (1 - B^m)y_t
$$

And when both are needed — as with `mexretail` — the two operators simply multiply:

$$
(1-B)(1-B^m)y_t
$$

which is exactly the expression we expanded by hand in the previous section.

| Operation | Backshift form |
|:----------|:--------------|
| First difference | $(1 - B)\,y_t$ |
| Second difference | $(1 - B)^2\,y_t$ |
| $d$-th difference | $(1 - B)^d\,y_t$ |
| Seasonal difference | $(1 - B^m)\,y_t$ |
| Both together | $(1 - B)^d(1 - B^m)\,y_t$ |

:::

:::{.fragment}
:::{.callout-tip appearance="simple"}
## Order of differencing — revisited

Using backshift notation, it becomes trivial to see that the order doesn't matter. Both orderings produce the same expression, since multiplication is commutative:

$$
(1-B)(1-B^m)y_t = (1-B^m)(1-B)y_t
$$

No algebra needed.
:::
:::

## Applying This to `mexretail`

`mexretail` has trend, seasonality, and growing variance. The full differencing pipeline:

::: {.panel-tabset}

### Levels

```{r}
#| label: mex-levels
#| echo: false
#| message: false
#| output: false

mex_levels_p <- mexretail |>
  autoplot(y) +
  labs(title = "Levels — trend + seasonality + growing variance",
       y = "Index 2015=100", x = "")
```

```{r}
#| label: mex-levels-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mex_levels_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mex_levels_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```

### Log

```{r}
#| label: mex-log
#| echo: false
#| message: false
#| warning: false
#| output: false

mex_log_p <- mexretail |>
  autoplot(log(y)) +
  labs(title = "Log — variance partially stabilized, trend + seasonality remain",
       y = "log(Index)", x = "")
```

```{r}
#| label: mex-log-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mex_log_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mex_log_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```

### Box-Cox

```{r}
#| label: mex-boxcox
#| echo: false
#| message: false
#| warning: false
#| output: false

mex_boxcox_p <- mexretail |>
  autoplot(box_cox(y, lambda)) +
  labs(
    title = glue::glue("Box-Cox (λ = {round(lambda, 3)}) — variance better stabilized"),
    y = "box_cox(y, λ)", x = ""
  )
```

```{r}
#| label: mex-boxcox-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mex_boxcox_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mex_boxcox_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```

### Seasonal diff

```{r}
#| label: mex-sdiff
#| echo: false
#| message: false
#| warning: false
#| output: false

mex_sdiff_p <- mexretail |>
  autoplot(difference(box_cox(y, lambda), 12)) +
  labs(title = "Seasonal diff of Box-Cox — trend largely removed",
       y = "Δ₁₂ box_cox(y)", x = "")
```

```{r}
#| label: mex-sdiff-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mex_sdiff_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mex_sdiff_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```

### Seasonal + first diff

```{r}
#| label: mex-ddiff
#| echo: false
#| message: false
#| warning: false
#| output: false

mex_ddiff_p <- mexretail |>
  autoplot(difference(box_cox(y, lambda), 12) |> difference(1)) +
  labs(title = "Seasonal + first diff of Box-Cox — approximately stationary",
       y = "Δ₁Δ₁₂ box_cox(y)", x = "")
```

```{r}
#| label: mex-ddiff-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mex_ddiff_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mex_ddiff_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```

:::

:::{.fragment}
:::{.callout-note appearance="simple"}
## 

We have been deciding how many differences to apply by looking at plots. But visual inspection is subjective — two analysts could disagree. Is there a more rigorous, objective way to make this call?

:::
:::

# Unit Root Tests

Visual inspection is useful but subjective. **Unit root tests** formalize the question: *is this series stationary?*

The test we will use is the **KPSS test** (Kwiatkowski-Phillips-Schmidt-Shin).

:::{.content-visible unless-format="revealjs"}
:::{.callout-note collapse="true"}
## The KPSS statistic

The KPSS test works by regressing $y_t$ on a constant (or a constant + trend), computing the residuals $e_t$, and then building their cumulative sums $S_t = \sum_{i=1}^{t} e_i$. The test statistic is:

$$\text{KPSS} = \frac{1}{T^2} \sum_{t=1}^{T} \frac{S_t^2}{\hat{\sigma}^2}$$

where $\hat{\sigma}^2$ is a long-run variance estimate that corrects for autocorrelation in the residuals.

The intuition: if the series is stationary, the residuals $e_t$ fluctuate around zero and their cumulative sums $S_t$ stay bounded. If the series has a unit root (non-stationary), the cumulative sums drift away from zero — making the statistic large and leading to rejection of $H_0$.
:::
:::

:::{.fragment}
:::{.callout-important appearance="simple"}
## Read the hypotheses carefully — they are the opposite of what you might expect

$$
H_0: \text{the series IS stationary}
$$
$$
H_1: \text{the series is NOT stationary}
$$

- **Large p-value** ($> 0.05$): fail to reject $H_0$ → the series is stationary ✓
- **Small p-value** ($< 0.05$): reject $H_0$ → the series needs differencing ✗

You *want* a large p-value here.

Because you want it, a large p-value on a short series deserves suspicion: a
weak test fails to reject whether or not the series is stationary. See
[power](../../../more/significance.qmd#sec-power).
:::
:::



## KPSS in R

```{r}
#| label: kpss-google
#| echo: true

google_2015 |> features(Close, unitroot_kpss)
```

```{r}
#| label: kpss-returns
#| echo: true

google_2015 |> features(diff_close, unitroot_kpss)
```

## Automated Differencing Orders

Rather than manually testing each transformation, `feasts` provides two functions that determine exactly how many differences are needed:

```{r}
#| label: nsdiffs-mexretail
#| echo: true

mexretail |> features(box_cox(y, lambda), unitroot_nsdiffs)
```

```{r}
#| label: ndiffs-mexretail
#| echo: true

mexretail |>
  features(difference(box_cox(y, lambda), 12), unitroot_ndiffs)
```

:::{.fragment}
:::{.callout-tip collapse="true"}
## Full stationarity protocol for `mexretail`

1. Apply Box-Cox with Guerrero lambda to stabilize variance: `box_cox(y, lambda)`
2. Check seasonal differencing needed: `unitroot_nsdiffs()` → 1
3. Apply seasonal diff, then check regular: `unitroot_ndiffs()` → 1
4. Final stationary series: `difference(box_cox(y, lambda), 12) |> difference(1)`
:::
:::

# ACF and PACF

We have already encountered the **ACF** when diagnosing residuals from our benchmark models — checking whether leftover errors looked like white noise. Here we use it for a different but related purpose: understanding the correlation structure of the series itself.

## Autocorrelation Function (ACF)

The **ACF** measures the correlation between a series and its own past values at each lag $k$:

$$r_k = \text{Corr}(y_t,\, y_{t-k})$$

:::{.fragment}
:::{.callout-note appearance="simple"}
## ACF as a stationarity diagnostic

- **Stationary** series: ACF drops to zero **quickly**.
- **Non-stationary** series: ACF decays **very slowly**, with the lag-1 value close to 1.

::: {.fragment}
This gives us another way to detect non-stationarity — and, as we will see, it also tells us about the structure of the model to fit.
:::
:::
:::

## ACF Shapes for Different Patterns

::: {.panel-tabset}

### Trend only

```{r}
#| label: acf-trend
#| echo: false
#| message: false
#| output: false

acf_trend_p <- google_2015 |> ACF(Close) |> autoplot() +
  labs(title = "ACF: Google stock price (trend) — very slow decay")
```

```{r}
#| label: acf-trend-render
#| renderings: [light, dark]
#| echo: false

acf_trend_p + theme_narsil()
acf_trend_p + theme_narsil_dark()
```

### Seasonality only

```{r}
#| label: acf-season-only
#| echo: false
#| message: false
#| warning: false
#| output: false

acf_season_only_p <- mexretail |>
  model(stl = STL(box_cox(y, lambda) ~ trend(window = NULL), robust = TRUE)) |>
  components() |>
  mutate(season_only =  `box_cox(y, lambda)` - trend) |>
  ACF(season_only, lag_max = 48) |>
  autoplot() +
  labs(title = "ACF: mexretail detrended (seasonality only) — repeating pattern at seasonal lags")
```

```{r}
#| label: acf-season-only-render
#| renderings: [light, dark]
#| echo: false

acf_season_only_p + theme_narsil()
acf_season_only_p + theme_narsil_dark()
```

### Trend + seasonality

```{r}
#| label: acf-trend-season
#| echo: false
#| message: false
#| output: false

acf_trend_season_p <- mexretail |> ACF(y, lag_max = 48) |> autoplot() +
  labs(title = "ACF: mexretail levels (trend + seasonality) — slow decay with seasonal bumps")
```

```{r}
#| label: acf-trend-season-render
#| renderings: [light, dark]
#| echo: false

acf_trend_season_p + theme_narsil()
acf_trend_season_p + theme_narsil_dark()
```

### Stationary

```{r}
#| label: acf-stationary
#| echo: false
#| message: false
#| output: false

acf_stationary_p <- google_2015 |> ACF(diff_close) |> autoplot() +
  labs(title = "ACF: Google daily returns (stationary) — drops to zero quickly")
```

```{r}
#| label: acf-stationary-render
#| renderings: [light, dark]
#| echo: false

acf_stationary_p + theme_narsil()
acf_stationary_p + theme_narsil_dark()
```

### mexretail after differencing

```{r}
#| label: acf-mexretail-diff
#| echo: false
#| message: false
#| warning: false
#| output: false

acf_mexretail_diff_p <- mexretail |>
  ACF(difference(box_cox(y, lambda), 12) |> difference(1)) |>
  autoplot() +
  labs(title = "ACF: Δ₁Δ₁₂ box_cox(mexretail) — stationary, with remaining structure to model")
```

```{r}
#| label: acf-mexretail-diff-render
#| renderings: [light, dark]
#| echo: false

acf_mexretail_diff_p + theme_narsil()
acf_mexretail_diff_p + theme_narsil_dark()
```

:::

## Partial Autocorrelation Function (PACF)

The ACF at lag $k$ captures both the *direct* relationship between $y_t$ and $y_{t-k}$ and the *indirect* relationship mediated through intermediate lags. The **PACF** isolates only the direct part.

:::{.fragment}
:::{.callout-tip appearance="simple"}
## The regression analogy

- **ACF at lag $k$**: simple correlation between $y_t$ and $y_{t-k}$.
- **PACF at lag $k$**: the coefficient on $y_{t-k}$ in a regression of $y_t$ on $y_{t-1}, y_{t-2}, \ldots, y_{t-k}$.

::: {.fragment}
The PACF asks: *once I already know the effect of all closer lags, does lag $k$ add any new information?*
:::
:::
:::

## ACF and PACF Together

`gg_tsdisplay()` with `plot_type = "partial"` shows the time series, ACF, and PACF together — this is the standard diagnostic display for the rest of the module.

```{r}
#| label: tsdisplay-mexretail
#| warning: false
#| message: false
#| results: hide

theme_narsil()
mexretail |>
  gg_tsdisplay(
    difference(box_cox(y, lambda), 12) |> difference(1),
    plot_type = "partial",
    lag_max = 48
  )
```

# AR and MA Models


## Autoregressive Models — AR($p$)

An **autoregressive model** of order $p$ forecasts $y_t$ as a weighted sum of its own **past values**:

$$
y_t = c + \phi_1 y_{t-1} + \phi_2 y_{t-2} + \cdots + \phi_p y_{t-p} + \varepsilon_t
$$

- Structurally identical to multiple linear regression — except the predictors are *lagged values of the series itself*.
- $p$ is the number of past values used.
- $\varepsilon_t$ is white noise.


```{r}
#| label: ar-ma-sims
#| echo: false
#| message: false
#| warning: false
#| output: false

set.seed(42)
ar1_sim <- arima.sim(list(order = c(1, 0, 0), ar = 0.8), n = 150) |>
  as_tsibble()
ar2_sim <- arima.sim(list(order = c(2, 0, 0), ar = c(1.3, -0.7)), n = 150) |>
  as_tsibble()

ma1_sim <- arima.sim(list(order = c(0, 0, 1), ma = 0.9), n = 500) |>
  as_tsibble()
ma2_sim <- arima.sim(list(order = c(0, 0, 2), ma = c(0.8, -0.7)), n = 500) |>
  as_tsibble()

# ar1_sim |>
#   gg_tsdisplay(value, plot_type = "partial", lag_max = 20 ) +
#   ggtitle("AR(1) ACF/PACF")
#
# ar2_sim |>
#   gg_tsdisplay(value, plot_type = "partial", lag_max = 20 ) +
#   ggtitle("AR(2) ACF/PACF")
#
# ma1_sim |>
#   gg_tsdisplay(value, plot_type = "partial", lag_max = 20 ) +
#   ggtitle("MA(1) ACF/PACF")
#
# ma2_sim |>
#   gg_tsdisplay(value, plot_type = "partial", lag_max = 20 ) +
#   ggtitle("MA(2) ACF/PACF")
```

```{r}
#| label: ar1-example
#| echo: true
#| message: false
#| warning: false
#| fig-height: 4
#| output: false

ar1_examples_p <- ar1_sim |> autoplot(value) +
  labs(title = "Simulated AR(1): φ₁ = 0.8", x = "", y = "")
```

```{r}
#| label: ar1-example-render
#| renderings: [light, dark]
#| echo: false
#| fig-height: 4

ar1_examples_p + theme_narsil()
ar1_examples_p + theme_narsil_dark()
```

```{r}
#| label: ar2-example
#| echo: true
#| message: false
#| warning: false
#| fig-height: 4
#| output: false

ar2_examples_p <- ar2_sim |> autoplot(value) +
  labs(title = "Simulated AR(2): φ₁ = 1.3, φ₂ = −0.7", x = "", y = "")
```

```{r}
#| label: ar2-example-render
#| renderings: [light, dark]
#| echo: false
#| fig-height: 4

ar2_examples_p + theme_narsil()
ar2_examples_p + theme_narsil_dark()
```


:::{.fragment}
:::{.callout-note appearance="simple"}
## Signature in ACF/PACF

- **PACF** cuts off sharply after lag $p$ — this is how you read the order.
- **ACF** decays exponentially or sinusoidal.
:::
:::

```{r}
#| label: ar-acf-pacf
#| echo: false
#| output: false

# Theoretical ACF/PACF values
ar1_acf  <- ARMAacf(ar = 0.8,            lag.max = 20, pacf = FALSE)
ar1_pacf <- ARMAacf(ar = 0.8,            lag.max = 20, pacf = TRUE)
ar2_acf  <- ARMAacf(ar = c(1.3, -0.7),   lag.max = 20, pacf = FALSE)
ar2_pacf <- ARMAacf(ar = c(1.3, -0.7),   lag.max = 20, pacf = TRUE)
ma1_acf  <- ARMAacf(ma = 0.9,            lag.max = 20, pacf = FALSE)
ma1_pacf <- ARMAacf(ma = 0.9,            lag.max = 20, pacf = TRUE)
ma2_acf  <- ARMAacf(ma = c(1.2, -0.7),   lag.max = 20, pacf = FALSE)
ma2_pacf <- ARMAacf(ma = c(1.2, -0.7),   lag.max = 20, pacf = TRUE)

# Helper to tidy into a plottable dataframe
# ACF includes lag 0 (always 1), PACF does not — handle separately
tidy_acf <- function(x, type) {
  lags <- if (type == "ACF") as.integer(names(x)) else seq_along(x)
  tibble(lag = lags, value = as.numeric(x), type = type) |>
    filter(lag > 0)  # drop lag 0
}

make_model_df <- function(acf_vals, pacf_vals, model_label) {
  bind_rows(
    tidy_acf(acf_vals,  "ACF"),
    tidy_acf(pacf_vals, "PACF")
  ) |> mutate(model = model_label)
}

df <- bind_rows(
  make_model_df(ar1_acf,  ar1_pacf,  "AR(1)"),
  make_model_df(ar2_acf,  ar2_pacf,  "AR(2)")
) |>
  mutate(model = factor(model, levels = c("AR(1)", "AR(2)")))


df2 <- bind_rows(
  make_model_df(ma1_acf,  ma1_pacf,  "MA(1)"),
  make_model_df(ma2_acf,  ma2_pacf,  "MA(2)")
) |> 
  mutate(model = factor(model, levels = c("MA(1)", "MA(2)")))

# Approximate 95% confidence bound (theoretical, large-sample)
ci <- 1.96 / sqrt(500)

ar_acf_pacf_p <- ggplot(df, aes(x = lag, y = value)) +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = c(-ci, ci), linetype = "dashed", colour = "#D55E00") +
  geom_segment(aes(xend = lag, yend = 0), linewidth = 0.8) +
  geom_point(size = 1.5) +
  facet_grid(model ~ type) +
  labs(x = "Lag", y = NULL,
       title = "Theoretical ACF and PACF signatures")
```

```{r}
#| label: ar-acf-pacf-render
#| renderings: [light, dark]
#| echo: false

ar_acf_pacf_p + theme_narsil()
ar_acf_pacf_p + theme_narsil_dark()
```


## Moving Average Models — MA($q$)

A **moving average model** of order $q$ forecasts $y_t$ as a weighted sum of **past forecast errors**:

$$
y_t = c + \varepsilon_t + \theta_1 \varepsilon_{t-1} + \theta_2 \varepsilon_{t-2} + \cdots + \theta_q \varepsilon_{t-q}
$$

- The predictors are *past errors*, not past values of the series.
- $q$ is the number of past errors used.
- $\varepsilon_t$ is white noise.

:::{.fragment}
:::{.callout-warning appearance="simple"}
## MA models ≠ moving average smoothing

Do not confuse MA *models* with the moving average *smoothing* used in decomposition. Smoothing estimates the trend-cycle from past observed values. MA models use past *errors* to describe the correlation structure of the series.
:::
:::

:::{.fragment}
:::{.callout-note appearance="simple"}
## Signature in ACF/PACF

- **ACF** cuts off sharply after lag $q$ — the mirror image of the AR signature.
- **PACF** decays exponentially or sinusoidal.
:::
:::

```{r}
#| label: ma-acf-pacf
#| echo: false
#| output: false

ma_acf_pacf_p <- ggplot(df2, aes(x = lag, y = value)) +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = c(-ci, ci), linetype = "dashed", colour = "#D55E00") +
  geom_segment(aes(xend = lag, yend = 0), linewidth = 0.8) +
  geom_point(size = 1.5) +
  facet_grid(model ~ type) +
  labs(x = "Lag", y = NULL,
       title = "Theoretical ACF and PACF signatures")
```

```{r}
#| label: ma-acf-pacf-render
#| renderings: [light, dark]
#| echo: false

ma_acf_pacf_p + theme_narsil()
ma_acf_pacf_p + theme_narsil_dark()
```


:::{.fragment}
:::{.callout-important appearance="simple"}
## You cannot identify a model from the time plot alone

Now that you have seen AR and MA models: their time plots can look very similar to each other. The ACF and PACF are what distinguish them. This is fundamentally different from ETS, where the form of trend and seasonality in the time plot directly guides model selection — one of the key differences we will revisit when comparing these two families.
:::
:::

## What Comes Next

In practice, you will rarely use a pure AR or pure MA model. Real series typically need both. And most real series also need differencing before any of this applies.

Combining differencing, AR terms, and MA terms into a single unified model — and applying it systematically to `mexretail` — is the subject of the next class.

:::{.fragment}
:::{.callout-note appearance="simple"}
## Where we are in the bigger picture

- Stabilize **variance** → Box-Cox / log transformation ✓
- Stabilize the **mean** → differencing ✓  
- Detect **remaining correlation structure** → ACF and PACF ✓
- Understand the building blocks → AR and MA terms ✓

All the pieces are in place. Time to put them together.
:::
:::
