...
1. Module 1 baseline: STL + Drift + SNAIVE. Our benchmark — every model must beat this. See [1.4 Model Diagnostics and Advanced Forecasting](../../module_1/04_model_diagnostics/model_diagnostics.qmd).
...
mexretail_fc_plot_p <- mexretail_fc |>
  autoplot(mexretail |> filter(year(date) >= 2015), level = NULL) +
  labs(
    title    = "mexretail — all approaches compared",
    y        = "Index 2015=100",
    subtitle = "point forecasts only"
  )
```

```{r}
#| label: mexretail-fc-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(mexretail_fc_plot_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(mexretail_fc_plot_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
mexretail_fc_pres_p <- mexretail_fc |>
  autoplot(mexretail |> filter(year(date) >= 2015), level = NULL) +
  labs(
    title = "mexretail — all approaches compared",
    y     = "Index 2015=100"
...
```{r}
#| label: mexretail-fc-plot-pres-render
#| renderings: [light, dark]
#| echo: false

mexretail_fc_pres_p + scale_narsil()
mexretail_fc_pres_p + scale_narsil_dark()
```
...
```{r}
#| label: beer-train-plot
#| echo: false
#| output: false

beer_train_plot_p <- beer_train |>
  autoplot(y) +
  labs(
...
```{r}
#| label: beer-train-plot-render
#| renderings: [light, dark]
#| echo: false

beer_train_plot_p + theme_narsil()
beer_train_plot_p + theme_narsil_dark()
```
...
beer_fc_plot_p <- beer_fc |>
  autoplot(beer_full |> filter(year(Quarter) >= 2000), level = NULL) +
  labs(
...
```{r}
#| label: beer-fc-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(beer_fc_plot_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(beer_fc_plot_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
beer_fc_pres_p <- beer_fc |>
  autoplot(beer_full |> filter(year(Quarter) >= 2000), level = NULL) +
  labs(
    title = "Australian beer — all approaches compared",
    y     = "Megalitres"
  )
```

```{r}
#| label: beer-fc-plot-pres-render
#| renderings: [light, dark]
#| echo: false

beer_fc_pres_p + scale_narsil()
beer_fc_pres_p + scale_narsil_dark()
```
