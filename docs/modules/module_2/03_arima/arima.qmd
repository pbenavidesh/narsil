...
`ARIMA()` in `fable` can search for the best order automatically. We can consolidate all selection strategies into a single `mable` for direct comparison:
```{r}
...
1. Leaving `pdq()` unspecified triggers automatic stepwise order selection. We still fix `PDQ(0,0,0)` to keep the non-seasonal constraint. 
2. Evaluates all combinations of $p \in \{1,2,3\}$ and $q \in \{0,1,2\}$, returns the lowest AIC$_c$.
...
- **Non-seasonal lags (1–11):** 
  - ACF: shows a spike at lag 4 → try $q = 4$.
  - PACF: shows a spike at lag 3 → try $p = 3$.
- **Seasonal lags (12, 24, 36):**
  - ACF: spike at lag 12, and just barely at lag 24 → try $Q = 1$ or $Q = 2$.
  - PACF: spikes at lag 12, 24, and slightly at 36 and 48  → seasonal AR(2), AR(3), or even AR(4).
:::
:::
...
A better strategy: start with a parsimonious candidate like $ARIMA(0,1,1)(0,1,1)_{12}$ or $ARIMA(1,1,1)(1,1,1)_{12}$, fit a few of the larger candidates suggested by the plots, and let AIC$_c$ arbitrate. You will often find that the simpler model performs comparably or better — and it will always be more stable.
:::
:::
