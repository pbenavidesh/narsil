...
:::{.content-hidden unless-format="revealjs"}
## Full ARIMA in Backshift Notation
:::
...
```{r}
#| label: us-change-forecast-render
#| renderings: [light, dark]
#| echo: false

us_change_forecast_p + scale_narsil()
us_change_forecast_p + scale_narsil_dark()
```
...
```{r}
#| label: us-change-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(us_change_plot_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(us_change_plot_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
:::{.content-hidden unless-format="revealjs"}
## Fitting the Candidates
:::

::: {.fragment}
```{r}
#| label: us-change-fit-full
#| message: false
...
```{r}
#| label: stl-arima-forecast-render
#| renderings: [light, dark]
#| echo: false

stl_arima_forecast_p + scale_narsil()
stl_arima_forecast_p + scale_narsil_dark()
```
...
:::{.content-hidden unless-format="revealjs"}
:::{.callout-tip appearance="simple"}
## When the plots suggest large orders

Start with a parsimonious candidate — ARIMA(0,1,1)(0,1,1)₁₂ or ARIMA(1,1,1)(1,1,1)₁₂ — fit a few larger ones, and let AIC~c~ arbitrate. Simpler models are more stable and often perform comparably.
:::
:::
```{r}
#| label: sarima-fit
...
comparison_plot_facet_p <- mexretail_fit |>
  forecast(h = nrow(mexretail_test)) |>
...
```{r}
#| label: comparison-plot-facet-render
#| renderings: [light, dark]
#| echo: false

comparison_plot_facet_p + scale_narsil() + facet_wrap(vars(.model), ncol = 1)
comparison_plot_facet_p + scale_narsil_dark() + facet_wrap(vars(.model), ncol = 1)
```
...
comparison_plot_overlay_p <- mexretail_fit |>
  forecast(h = nrow(mexretail_test)) |>
  autoplot(mexretail |> filter(year(date) >= 2015), level = 80) +
  labs(
    title    = "mexretail — STL + ARIMA vs. SARIMA",
...
```{r}
#| label: comparison-plot-overlay-render
#| renderings: [light, dark]
#| echo: false

ggplotly(comparison_plot_overlay_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(comparison_plot_overlay_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
:::
...
::: {.column width="50%"}
```{r}
#| label: comparison-facet-pres
#| echo: false
#| output: false

comparison_facet_pres_p <- mexretail_fit |>
  forecast(h = nrow(mexretail_test)) |>
  autoplot(mexretail |> filter(year(date) >= 2015), level = 80) +
...
```{r}
#| label: comparison-facet-pres-render
#| renderings: [light, dark]
#| echo: false

comparison_facet_pres_p + scale_narsil()
comparison_facet_pres_p + scale_narsil_dark()
```
:::

::: {.column width="50%"}
```{r}
#| label: comparison-overlay-pres
#| echo: false
#| output: false

comparison_overlay_pres_p <- mexretail_fit |>
  forecast(h = nrow(mexretail_test)) |>
  autoplot(mexretail |> filter(year(date) >= 2015), level = 80) +
  labs(
    title    = "Overlay",
...
```{r}
#| label: comparison-overlay-pres-render
#| renderings: [light, dark]
#| echo: false

comparison_overlay_pres_p + scale_narsil()
comparison_overlay_pres_p + scale_narsil_dark()
```
:::
::::
