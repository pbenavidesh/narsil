...
Recall from the [previous topic](../02_stationarity/stationarity.html#backshift-notation) that the backshift operator $B$ satisfies $By_t = y_{t-1}$, and that $d$-th order differencing can be written as $(1-B)^d y_t$.
...
::: {.incremental}
1. **Choose $d$** — determine how many differences are needed using the stationarity protocol from the [previous topic](../02_stationarity/stationarity.html#unit-root-tests): Box-Cox / log transformation to stabilize variance, `unitroot_nsdiffs()` for seasonal differencing (if any), then `unitroot_ndiffs()` on the differenced series.
...
## Manual Specification {#sec-manual}
...
where $k = 1$ if the model includes a constant, $k = 0$ otherwise. The preferred criterion is AIC$_c$.
...
You can also supply a **range of values** to explore, or request an exhaustive search:
```{r}
...
2. `approximation = FALSE` uses exact likelihood rather than a faster approximation. More accurate but can be substantially slower on long series.
...
1. `dof` should equal $p + q$ — the number of estimated AR and MA coefficients. For ARIMA(3,0,0), `dof = 3`. Omitting this inflates the test statistic and produces misleadingly small p-values.
...
- **Non-seasonal lags (1–11):** ACF and PACF both show a spike at lag 1 → try $p = 1$ or $q = 1$.
- **Seasonal lags (12, 24, 36):** PACF spike at lag 12 only → seasonal AR(1). ACF spike at lag 12 → could also be seasonal MA(1). Start with both.
:::
:::
```{r}
#| label: sarima-fit
...
1. Set `dof` to $p + q + P + Q$ for the fitted model. Adjust this value to match the order reported by `report()` above.
...
mex_train <- mexretail |> filter(year(date) <= 2019)
mex_test  <- mexretail |> filter(year(date) >  2019)
```
```{r}
