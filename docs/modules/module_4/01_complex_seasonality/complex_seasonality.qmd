...
Back in [Module 3.3](../../module_3/03_dynamic/dynamic_regression.qmd), we modeled electricity demand using daily aggregated data. Temperature explained the U-shaped relationship with demand, and day-type dummies captured the weekday/weekend pattern.
...
full_series_plot_p <- vic_elec_hourly |>
...
```{r}
#| label: full-series-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(full_series_plot_p + theme_narsil(), dynamicTicks = TRUE) |>
  rangeslider() |>
  layout(yaxis = list(fixedrange = FALSE)) |>
  gg_to_plotly_narsil()

ggplotly(full_series_plot_p + theme_narsil_dark(), dynamicTicks = TRUE) |>
  rangeslider() |>
  layout(yaxis = list(fixedrange = FALSE)) |>
  gg_to_plotly_narsil(dark = TRUE)
```
...
season_daily_p <- vic_elec_hourly |>
...
```{r}
#| label: season-daily-render
#| renderings: [light, dark]
#| echo: false

ggplotly(season_daily_p + theme_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil()

ggplotly(season_daily_p + theme_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil(dark = TRUE)
```
...
season_weekly_p <- vic_elec_hourly |>
  filter_index("2014-01-01" ~ "2014-03-31") |>
...
```{r}
#| label: season-weekly-render
#| renderings: [light, dark]
#| echo: false

ggplotly(season_weekly_p + theme_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil()

ggplotly(season_weekly_p + theme_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: acf-plot-render
#| renderings: [light, dark]
#| echo: false

acf_plot_p + theme_narsil()
acf_plot_p + theme_narsil_dark()
```
...
stl_decomp_plot_p <- vic_elec_dcmp |>
  autoplot() +
  labs(title = "STL decomposition with two seasonal periods")
```

```{r}
#| label: stl-decomp-plot-render
#| renderings: [light, dark]
#| echo: false

stl_decomp_plot_p + theme_narsil()
stl_decomp_plot_p + theme_narsil_dark()
```
...
```{r}
#| label: stl-fc-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(stl_fc_plot_p + scale_narsil(), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil()

ggplotly(stl_fc_plot_p + scale_narsil_dark(), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil(dark = TRUE)
```
...
Recall from [Module 3.3](../../module_3/03_dynamic/dynamic_regression.qmd#harmonic-regression) that Fourier terms approximate a seasonal pattern with $K$ pairs of sine and cosine waves. For **multiple seasonalities**, we add one set per period:
...
```{r}
#| label: fourier-fc-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(fourier_fc_plot_p + scale_narsil(), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil()

ggplotly(fourier_fc_plot_p + scale_narsil_dark(), dynamicTicks = TRUE) |>
  gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: bank-eda-plot-render
#| renderings: [light, dark]
#| echo: false

ggplotly(bank_eda_plot_p + theme_narsil(), dynamicTicks = TRUE) |>
  rangeslider() |>
  layout(yaxis = list(fixedrange = FALSE)) |>
  gg_to_plotly_narsil()

ggplotly(bank_eda_plot_p + theme_narsil_dark(), dynamicTicks = TRUE) |>
  rangeslider() |>
  layout(yaxis = list(fixedrange = FALSE)) |>
  gg_to_plotly_narsil(dark = TRUE)
```
