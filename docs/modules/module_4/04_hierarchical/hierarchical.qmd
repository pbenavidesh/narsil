...
pbs_hts <- PBS |>
  ...
  aggregate_key(ATC1 / ATC2 * Concession, Cost = sum(Cost) / 1e6)
...
```{r}
#| label: hts-state-plot-render
#| renderings: [light, dark]
#| echo: false

hts_state_plot_p + theme_narsil() + theme(legend.position = "none")
hts_state_plot_p + theme_narsil_dark() + theme(legend.position = "none")
```
...
```{r}
#| label: full-hts-plot-render
#| renderings: [light, dark]
#| echo: false

full_hts_plot_p + theme_narsil() + theme(legend.position = "none")
full_hts_plot_p + theme_narsil_dark() + theme(legend.position = "none")
```
...
```{r}
#| label: reconcile-bu-render
#| renderings: [light, dark]
#| echo: false

reconcile_bu_p + scale_narsil()
reconcile_bu_p + scale_narsil_dark()
```
...
```
1. Average of $y_{j,t}/y_{Total,t}$ over time.
...
```{r}
#| label: accuracy-all-render
#| renderings: [light, dark]
#| echo: false

accuracy_all_p + scale_narsil() + scale_fill_narsil()
accuracy_all_p + scale_narsil_dark() + scale_fill_narsil(dark = TRUE)
```
...
#| label: pbs-accuracy
#| message: false
#| fig-cap: "Mean RMSSE for the PBS mixed hierarchy by method and aggregation level."
#| output: false
...
```{r}
#| label: pbs-accuracy-render
#| renderings: [light, dark]
#| echo: false

pbs_accuracy_p + scale_narsil() + scale_fill_narsil()
pbs_accuracy_p + scale_narsil_dark() + scale_fill_narsil(dark = TRUE)
```
