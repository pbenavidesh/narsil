...
format:
  html:
    other-links:
      - text: "FPP3 §5.10: Time series cross-validation"
        href: https://otexts.com/fpp3/tscv.html
      - text: "FPP3 §7.5: Selecting predictors (CV statistic)"
        href: https://otexts.com/fpp3/selecting-predictors.html
  revealjs:
    output-file: ts_cv_pres.html
freeze: auto
params:
  fred-data: true
---
...
## Same Models, Two Cutoffs, Two Winners
...
Chronological train/test ordering — training data always earlier, test data always later, never shuffled — was established in [Module 1.3](../../module_1/03_fcst/forecasting.qmd#train-test-split). Rolling-origin evaluation keeps that rule intact; it just repeats the split many times, sliding the origin forward through the series instead of fixing it once.
...
## Expanding vs. Sliding: A Quick Comparison
...
```{r}
#| label: fold-diagram-data
#| echo: false
#| output: false

fold_n <- 20

expanding_folds_df <- map_dfr(1:4, function(i) {
  train_end <- 8 + (i - 1) * 3
  test_end  <- train_end + 2
  tibble(t = 1:fold_n, fold = i) |>
...
sliding_folds_df <- map_dfr(1:4, function(i) {
  start     <- 1 + (i - 1) * 3
  train_end <- start + 7
  test_end  <- train_end + 2
  tibble(t = 1:fold_n, fold = i) |>
...
fold_diagram_tile <- function(.df, .title) {
  .df |>
    mutate(status = factor(status, levels = c("train", "test", "unused"))) |>
    ggplot(aes(x = t, y = factor(fold), fill = status)) +
    geom_tile(colour = "white", linewidth = 0.3) +
    labs(title = .title, x = "Time index", y = "Fold", fill = NULL)
}

fold_diagram_p <- (fold_diagram_tile(expanding_folds_df, "Expanding window") +
                      fold_diagram_tile(sliding_folds_df, "Sliding window")) +
  plot_layout(guides = "collect")
```

```{r}
#| label: fold-diagram-render
...
fold_diagram_p &
  ...
  scale_fill_manual(values = c(
    train  = narsil_pal("light")$palette[1],
    test   = narsil_pal("light")$palette[4],
    unused = narsil_pal("light")$grid
  )) &
...
fold_diagram_p &
  theme_narsil_dark() &
  scale_fill_manual(values = c(
    train  = narsil_pal("dark")$palette[1],
    test   = narsil_pal("dark")$palette[4],
    unused = narsil_pal("dark")$grid
  )) &
  theme(legend.position = "bottom")
```
...
```{r}
#| label: cv-horizon-plot-html
#| echo: false

p <- canadian_gas_cv_accu |>
  ggplot(aes(x = h, y = RMSE, colour = .model)) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 1.6) +
  labs(
    title  = "RMSE by forecast horizon — expanding-window CV",
    x      = "Horizon (months)",
    y      = "RMSE",
    colour = "Model"
  )
...
```{r}
#| label: cv-horizon-plot-revealjs
#| echo: false
...
# The Real Cost of Cross-Validating Every Model

## Why This Gets Expensive Fast

```{r}
#| label: naive-cv-illustration
#| eval: false

mexretail_full_cv <- mexretail_train |>
  stretch_tsibble(.step = 1, .init = 120, .id = ".id") |>
  model(
    ets     = ETS(log(y)),
    arima   = ARIMA(log(y)),
    stlf    = stlf_spec,
...
cg_stretch_step12 <- canadian_gas_recent |>
  stretch_tsibble(.step = 12, .init = 100, .id = ".id")

time_step1  <- system.time(cg_stretch_step1  |> model(ets = ETS(Volume)) |> forecast(h = 12))
time_step12 <- system.time(cg_stretch_step12 |> model(ets = ETS(Volume)) |> forecast(h = 12))
...
canadian_gas |> stretch_tsibble(.step = 12, .init = 120, .id = ".id") # 36 folds
canadian_gas |> stretch_tsibble(.step = 1,  .init = 120, .id = ".id") # 423 folds
```
...
```
1. Same split as [Module 4.2](../02_bootstrap_combinations/bootstrap_combinations.qmd): training through 2017, the last two years held out.

```{r}
#| label: mexretail-specs
...
mable_spec <- function(.train_tsb) {   #<1>
  .train_tsb |>
    model(
      tslm         = tslm_spec,
      ets          = ets_spec,
...
```
1. Same seven-model roster as Module 4.2, wrapped in a function so it can be refit on any training tsibble — the single split now, the CV folds below.

```{r}
#| label: mexretail-single-split
#| cache: true

mexretail_fit <- mexretail_train |> mable_spec()
...
`arima`, `comb_inv_var`, `prophet`, and `ets` all land under RMSE 2.5. `stlf` (3.72) and `tslm` (11.3) are well behind. Carrying only the top four into cross-validation instead of all seven cuts the fold × model workload nearly in half.
...
## mexretail: Does the 4.2 Winner Still Win?
...
## Leave-One-Out CV for Selecting Predictors
