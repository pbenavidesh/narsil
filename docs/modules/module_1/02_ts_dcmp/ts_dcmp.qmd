...
```{r}
#| label: jj_levels_render
#| renderings: [light, dark]
#| echo: false

ggplotly(jj_levels_p + scale_narsil()) |> gg_to_plotly_narsil()
ggplotly(jj_levels_p + scale_narsil_dark()) |> gg_to_plotly_narsil(dark = TRUE)
```
...
ggplotly(jj_log_p + scale_narsil()) |> gg_to_plotly_narsil()
ggplotly(jj_log_p + scale_narsil_dark()) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: calendar_google_render
#| renderings: [light, dark]
#| echo: false

ggplotly(calendar_p + scale_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(calendar_p + scale_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: google_monthly_vol_render
#| renderings: [light, dark]
#| echo: false

ggplotly(google_vol_p + scale_narsil() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil() |> rangeslider() |> layout(yaxis = list(fixedrange = FALSE))
ggplotly(google_vol_p + scale_narsil_dark() + theme(legend.position = "none"), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE) |> rangeslider() |> layout(yaxis = list(fixedrange = FALSE))
```
...
```{r}
#| label: global_economy_plotly
#| echo: false
#| output: false

global_economy_p <- ge |>
  autoplot(GDP, linewidth = 1) +
  labs(title = "GDP: Mexico, Iceland, and Australia")
```

```{r}
#| label: global_economy_render
#| renderings: [light, dark]
#| echo: false

ggplotly(global_economy_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(global_economy_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
population_p <- ge |>
  autoplot(Population, linewidth = 1) +
  labs(
...
```{r}
#| label: population_render
#| renderings: [light, dark]
#| echo: false

ggplotly(population_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(population_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: gdp_per_capita_render
#| renderings: [light, dark]
#| echo: false

ggplotly(gdpcap_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(gdpcap_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
## Business examples of population adjustments {.smaller}

:::

::: {.content-visible unless-format="revealjs"}

### Business examples of population adjustments
...
- Per-customer series tracks **ARPA / LTV direction** (signal, not hype)
-**Normalize by:** active customers
:::
...
- Tickets per customer exposes **product/service degradation**
-**Normalize by:** customers or contracts
:::
:::
...
```{r}
#| label: inflation_adjustment_render
#| renderings: [light, dark]
#| echo: false

print_retail_p + scale_narsil()
print_retail_p + scale_narsil_dark()
```
### Real values
...
inflation_compare_p <- print_retail |>
  pivot_longer(
...
```{r}
#| label: inflation_compare_render
#| renderings: [light, dark]
#| echo: false

ggplotly(inflation_compare_p + scale_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(inflation_compare_p + scale_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: seasonally_adjusted
#| echo: false
#| output: false

mexretail_stl <-  mexretail |>
  model(stl = STL(y, robust = TRUE)) |>
  components()

sa_light_p <- mexretail_stl |>
  ggplot(aes(x = date, y = y)) +
  geom_line(colour = "#B8C8D8") +
  geom_line(aes(y = season_adjust), colour = "#1A5276") +
  labs(
    title = "Retail Trade, Except of Motor Vehicles and Motorcycles for Mexico",
    y = "Index 2015=100",
    x = ""
...
sa_dark_p <- mexretail_stl |>
  ggplot(aes(x = date, y = y)) +
  geom_line(colour = "#4A5468") +
  geom_line(aes(y = season_adjust), colour = "#E8651A") +
  labs(
    title = "Retail Trade, Except of Motor Vehicles and Motorcycles for Mexico",
    y = "Index 2015=100",
    x = ""
...
```{r}
#| label: seasonally_adjusted_render
#| renderings: [light, dark]
#| echo: false

ggplotly(sa_light_p + theme_narsil(), dynamicTicks = TRUE) |> gg_to_plotly_narsil()
ggplotly(sa_dark_p + theme_narsil_dark(), dynamicTicks = TRUE) |> gg_to_plotly_narsil(dark = TRUE)
```
...
```{r}
#| label: classical_decomp_plot_render
#| renderings: [light, dark]
#| echo: false

mexretail_dcmp_p + scale_narsil()
mexretail_dcmp_p + scale_narsil_dark()
```
...
```{r}
#| label: stl_decomp_render
#| renderings: [light, dark]
#| echo: false

stl_dcmp_p + scale_narsil()
stl_dcmp_p + scale_narsil_dark()
```
